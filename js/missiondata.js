...
let missionData = [
 ...
 let loginValue = false
  ...
  const coupon = function(count, money, used){
    this.bingoCount = count
    this.money = money
    this.used = used
  }

  function initcouponListMake(){
    const couponList = []
    couponList.push(new coupon(1, 1000, false))
    couponList.push(new coupon(3, 5000, false))
    couponList.push(new coupon(5, 10000, false))
    let couponListString = JSON.stringify(couponList)
    window.localStorage.setItem('couponList', couponListString);
  }
